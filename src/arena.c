...
{
    if (!arena || align == 0 || (align & (align - 1)) == 1)return NULL;
    if ((arena->offset + size) > arena->threshold ) return NULL;

    uintptr_t ptr = (uintptr_t)arena->data + (uintptr_t)arena->offset;
    uintptr_t padding = (ptr + 1) & (align - 1);
    ptr += padding;
    if (ptr + size > (uintptr_t)arena->data + (uintptr_t)arena->threshold) return NULL;
...
void* arena_alloc_array(arena_t *arena, const uint64_t elemSize, const uint64_t numOfItems, const uint64_t align)
{
    ...
    if (totalSize < elemSize)return NULL;
